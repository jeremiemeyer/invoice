 ...
 */
export const setIsHtmlMode = (value: boolean) => {
  isHtml = value;
};
 ...
 */
export const getIsHtmlMode = () => isHtml;
...
const fontWeightConverter = (fontWeight?: fontWeight | string | number) => {
...
const adjustStyles = (style: Style) => {
  ...
  Object.keys(style).forEach((key) => {
    if (key === "paddingVertical") {
      ...
      style.marginTop = style[key];
      style.marginBottom = style[key];
    } else if (key === "marginHorizontal") {
      style.marginLeft = style[key];
      style.marginRight = style[key];
...
export const CustomView: FC<PropsView> = ({ children, style, ...rest }) => {
  ...
  // In PDF mode, don't add default position/left/right styles
  // as they can interfere with React-PDF's layout engine
  return (
